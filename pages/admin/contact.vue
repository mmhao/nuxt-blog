...
<script>
const {singleDetail,updateSingle} = require('interface/single')
export default {
  ...
  mounted() {
    ...
    this.style = aRouter.splice(1, 1).join('')

    return singleDetail({params: {style: this.style}}).then((resp)=>{
      let reapData = resp.data
      if (reapData.data) {
        this.formData = reapData.data
      }
    })
  },
  methods: {
    ...
    onSubmit () {
      ...
      _this.$refs.formRef.validate((valid) => {
          if (valid) {
            ...
            updateSingle(reqData).then((resp)=>{
